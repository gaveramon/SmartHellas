...
-- 004_DEVICE_TELEMETRY.SQL
--
-- Purpose:
-- Raw device telemetry ingestion and immutable storage
--
-- Authority:
-- 003_property_device_engine.sql = DEVICE SSOT
--
-- SSOT RULE:
...
--
-- device_id references the 003 device registry.
...
-- 004 must not impose a semantic interpretation on the
-- provider payload.
-- =====================================================
...
--
-- 003 is the SSOT for device ownership.
--
...
comment on column public.device_telemetry_raw.tenant_id is
    'Tenant ownership copied from the 003 device SSOT for isolation and indexed tenant access. Must match devices.tenant_id.';


comment on column public.device_telemetry_raw.device_id is
    'Reference to the 003 device registry SSOT.';
...
-- =====================================================
-- END 004 DEVICE TELEMETRY
--
-- SSOT BOUNDARY:
--
-- 003 = Device/domain registry SSOT
-- 004 = Raw telemetry input SSOT
--
