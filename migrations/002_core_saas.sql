...
create or replace function platform.resolve_membership(
    p_user_id uuid,
...
as $$
    select m.tenant_id
    from platform.resolve_membership(p_user_id, p_verify_tenant_id) m
    limit 1;
$$;
...
as $$
    select m.role
    from platform.resolve_membership((select auth.uid()), null) m
    limit 1;
$$;
...
begin
    ...
    into v_row
    from platform.resolve_membership(p_user_id, p_target_tid) m
    limit 1;
...
alter function platform.resolve_membership(uuid, uuid) set search_path = '';
