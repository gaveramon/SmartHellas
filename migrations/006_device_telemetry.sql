...
-- 006 must not impose semantic interpretation on the
-- provider payload.
-- =====================================================
...
-- =====================================================
-- 2A. IDEMPOTENCY CONSTRAINT
--
-- provider_event_id is the provider-native event identity.
--
-- When a provider supplies an event identifier, the same
-- event must not be stored more than once for the same
-- source.
--
-- NULL provider_event_id values remain allowed because
-- some telemetry sources may not provide an event ID.
--
-- IMPORTANT:
-- This assumes source + provider_event_id uniquely
-- identifies an event within the provider/source boundary.
-- =====================================================

create unique index if not exists uq_device_telemetry_raw_provider_event
on public.device_telemetry_raw (
    source,
    provider_event_id
)
...
comment on column public.device_telemetry_raw.source is
    'Raw telemetry source/provider identifier. Provider identity resolution belongs to the integration layer.';


comment on column public.device_telemetry_raw.provider_event_id is
    'Optional provider-native event identifier. When present, source + provider_event_id uniquely identifies the raw event and enforces ingestion idempotency.';
...
-- 5A. SELECT
--
-- Tenant access is enforced against the row's tenant_id.
-- Platform administrators may access all tenants according
...
--
-- Idempotency:
-- When provider_event_id is supplied, source +
-- provider_event_id prevents duplicate raw events.
-- If the event already exists, its existing row ID is
-- returned.
-- =====================================================
...
begin
    ...
    -- IDEMPOTENT INSERT
    --
    -- If source + provider_event_id already exists,
    ...
    -- EXISTING EVENT
    --
    -- If the insert was skipped because the event already
    -- exists, return the existing canonical row ID.
    -- =================================================

    if v_id is null
       and p_provider_event_id is not null then

        select r.id
        into v_id
        from public.device_telemetry_raw r
    ...
    if v_id is null then
        raise exception
            'telemetry ingestion failed to resolve row id';
    end if;
...
comment on function public.ingest_raw_device_telemetry(
    ...
    'Trusted server-side ingestion of immutable raw provider telemetry. Device identity must already be resolved by the integration layer. When provider_event_id is supplied, ingestion is idempotent by source + provider_event_id.';
