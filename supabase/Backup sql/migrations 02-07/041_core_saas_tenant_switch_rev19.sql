...
-- -----------------------------------------------------
-- Membership SSOT (user + tenant; no JWT dependency)
-- -----------------------------------------------------
...
comment on function platform.has_tenant_membership(uuid, uuid) is
    'Active tenant_memberships row for user+tenant. Authorization SSOT — does not read JWT tenant_id.';
...
-- -----------------------------------------------------
-- Shared switch validation (membership before JWT write)
-- -----------------------------------------------------
...
begin
    ...
    if not platform.has_tenant_membership(p_user_id, p_target_tid) then
    ...
    where tm.tenant_id = p_target_tid
